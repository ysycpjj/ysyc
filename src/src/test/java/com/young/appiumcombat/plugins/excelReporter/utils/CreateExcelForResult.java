package com.young.appiumcombat.plugins.excelReporter.utils;
import java.io.File;
...
public class CreateExcelForResult{
	...
	public static Logger L = Logger.getLogger(CreateExcelForResult.class);
	/**excel文件声明*/
	...
	static final String OBJECT_CONFIGURATION_PATH ="res/properties/config.properties";
	static String logPath = PropertiesDataProvider.getTestData(OBJECT_CONFIGURATION_PATH, "logPath");
	static String log4jConfigurationPath = PropertiesDataProvider.getTestData(OBJECT_CONFIGURATION_PATH, "log4jConfigPath");
	static String screenShotPath = PropertiesDataProvider.getTestData(OBJECT_CONFIGURATION_PATH, "screenShotPath");
...
// 					String caseName = im.getTestClass().getRealClass().getSimpleName();
					String caseName = im.getMethodName();
					String className = im.getTestClass().getRealClass().getSimpleName();
// 					String functionPack = caseName.substring(0, caseName.indexOf("P")).toLowerCase();
 					String functionPack = className.substring(0, className.indexOf("P")).toLowerCase();
 					//设置用例名字
					...
					//("HYPERLINK(\"" +new File("result/logs/"+functionPack+"/"+className+".log").getAbsolutePath()+ "\",\"" +className+ ".log\")");
					workbook.getSheet(function).getRow(count+2).getCell(3).setCellFormula
					("HYPERLINK(\"" +new File(logPath+functionPack+"/"+className+".log").getAbsolutePath()+ "\",\"" +className+ ".log\")");
			        ...
			        //添加截图,截图名称中添加了时间戳，遍历该Test截图所在的包，找到名字包含方法名的，并拿到名字，包含后缀。
			        long  currentTime=0,maxTime=0;
//			        File file = new File("result/screenshots/"+className+"//");
			        File file = new File(screenShotPath+className+"//");
			        File[] fileList = file.listFiles();
//			        result/screenshots/TaxGuidePage_001_UiCheck_test/
//			        result/screenshot/TaxGuidePage_001_UiCheck_test
			        if(fileList!=null){
				        for(int i=0;i<fileList.length;i++){
				        	if(fileList[i].getName().contains(caseName)){
				        		currentTime =Long.valueOf(fileList[i].getName().substring( fileList[i].getName().lastIndexOf("_")+1, fileList[i].getName().indexOf(".")));
				        		if(currentTime >maxTime )
				        			maxTime = currentTime;
				        	}
				        }
	//			        File png = new File("result/screenshots/"+className+"/"+caseName+"_"+String.valueOf(maxTime)+".png");
				        File png = new File(screenShotPath+className+"/"+caseName+"_"+String.valueOf(maxTime)+".png");
	//			        File png = new File("result/screenshot/"+className+"/"+caseName+".png");
				        if(png.exists()){
				        	//设置截图图片，这是超链接 result/screenshots/TaxGuidePage_001_UiCheck_test/DiscountRecordUiCheck_1494488254235.png
				        	//workbook.getSheet(function).getRow(count+2+failTestNum).getCell(4).setCellFormula
	//						("HYPERLINK(\"" +new File("result/screenshots/"+className+"/"+caseName+"_"+String.valueOf(maxTime)+".png").getAbsolutePath()+ "\",\""+className+"/"+caseName+String.valueOf(maxTime)+ ".png\")");
				        	workbook.getSheet(function).getRow(count+2).getCell(4).setCellFormula
							("HYPERLINK(\"" +new File(screenShotPath+className+"/"+caseName+"_"+String.valueOf(maxTime)+".png").getAbsolutePath()+ "\",\""+className+"/"+caseName+String.valueOf(maxTime)+ ".png\")");
				        }else{
...
//    					("HYPERLINK(\"" +new File("result/logs/"+functionPack+"/"+className+".log").getAbsolutePath()+ "\",\"" +className+ ".log\")");
        ...
        // 操作结束，关闭文件
			fos.close();
			L.info("Excel报告已经生成，请查看：["+fileForExcel.getAbsolutePath()+"]");
	    }catch(Exception e){
	    	e.printStackTrace();
	    }
	}
